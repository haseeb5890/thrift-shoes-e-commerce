...
export function notifyNewOrder(params: {
  ...
  items: { name: string; size: string; imageUrl: string }[]
}) {
  ...
  const blocks = [
    ...
    ...params.items.map((item) => ({
      type: "section",
      text: { type: "mrkdwn", text: `${item.name} · Size ${item.size}` },
...
export function notifyOrderConfirmed(params: { orderNumber: string; customerName: string; via: "customer" | "admin" }) {
  return postToSlack({
    text: `:white_check_mark: *Order confirmed ${params.orderNumber}* — ${params.customerName} (confirmed by ${params.via === "customer" ? "customer via email link" : "admin"})`,
  })
}
