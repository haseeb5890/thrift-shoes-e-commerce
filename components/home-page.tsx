...
export function HomePage({ products }: { products: Product[] }) {
  ...
  // 2. Safe calculation of the lowest price
  const lowestPrice = safeProducts.length 
    ? Math.min(...safeProducts.map((product) => product.price)) 
    : null
  ...
  return <>
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 md:px-6 md:py-4">
        <div className="grid items-center gap-6 md:grid-cols-[1fr_1.1fr_1fr]">
          ...
          <div className="relative mx-auto flex w-full max-w-xl items-center justify-center">
            ...
            <Image src="https://res.cloudinary.com/dnit1fib7/image/upload/v1784287744/hero-shoe_nq70h7.png" alt="Featured thrifted sneaker" width={2400} height={3000} priority className="relative z-10 h-[150%] w-full drop-shadow-2xl overflow-hidden"/>
          </div>
          <div className="text-center md:text-left">
    ...
    <section className="mx-auto max-w-7xl px-4 py-16 md:px-6 md:py-24"><div className="mb-9 flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">Freshly added</p><h2 className="mt-2 text-balance text-4xl font-black md:text-5xl">The latest rotation</h2></div><Link href="/shop" className="hidden items-center gap-2 text-sm font-bold md:flex">View all <ArrowRight size={16}/></Link></div>
