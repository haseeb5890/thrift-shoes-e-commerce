...
export function CheckoutForm() {
  ...
  // First pair costs the full per-city rate; each pair after that only adds 70% of it, since
  // they ship together in the same package rather than each costing a full separate delivery.
  const ADDITIONAL_ITEM_SHIPPING_RATE = 0.7
  const baseShipping = shippingRates[city] ?? shippingRates.Other
  const shipping = itemCount === 0 ? 0 : Math.round(baseShipping * (1 + ADDITIONAL_ITEM_SHIPPING_RATE * (itemCount - 1)))
